...
const udesc_client_1 = require("./udesc.client");
const zouwu_client_1 = require("./zouwu.client");
let SyncService = SyncService_1 = class SyncService {
    constructor(prisma, udescClient, zouwuClient) {
        this.prisma = prisma;
        this.udescClient = udescClient;
        this.zouwuClient = zouwuClient;
    ...
    async markIssueResolved(params) {
    ...
    resolveProviderEarliestDate(now) {
    ...
    estimateWindowCount(start, end, windowDays) {
    ...
    toWindowEnd(start, maxEnd, windowDays) {
    ...
    async syncUdesc() {
        ...
        try {
            ...
            while (windowStart <= finalEnd) {
                ...
                this.logger.log(`udesc sync window ${windowStart.toISOString()} -> ${windowEnd.toISOString()}`);
                let pageCursor = undefined;
                let hasMore = true;
                while (hasMore) {
                    const resp = await this.withRetry('udesc.fetchSessions', () => this.udescClient.fetchSessions({
                        cursor: pageCursor,
                        pageSize,
                    ...
                    for (const record of resp.records) {
                        ...
                        while (logHasMore && logPageCount < logMaxPagesPerSession) {
                            try {
                                const logResp = await this.withRetry('udesc.fetchSessionLogs', () => this.udescClient.fetchSessionLogs({
                                    sessionId: record.id,
                                    cursor: logCursor,
                ...
                this.progress.estimatedRemainingRecords = Math.max(0, Math.round(avgRecordsPerWindow * remainingWindows));
                this.progress.estimatedRemainingSeconds = Math.max(0, Math.round(avgSecondsPerWindow * remainingWindows));
            ...
            try {
                ...
                while (voteWindowStart > voteEarliest) {
                    ...
                    let voteCursor = undefined;
                    let voteHasMore = true;
                    while (voteHasMore) {
                        const voteResp = await this.withRetry('udesc.fetchSessionVotes', () => this.udescClient.fetchSessionVotes({
                            cursor: voteCursor,
                            pageSize: 100,
        ...
        `;
            ...
            try {
                let customerCursor = undefined;
                let customerHasMore = true;
                while (customerHasMore) {
                    const customerResp = await this.withRetry('udesc.fetchCustomers', () => this.udescClient.fetchCustomers({
            ...
            try {
                let agentCursor = undefined;
                let agentHasMore = true;
                while (agentHasMore) {
                    const agentResp = await this.withRetry('udesc.fetchAgents', () => this.udescClient.fetchAgents({
                        cursor: agentCursor,
                        pageSize: 100,
                    }));
            ...
            try {
                let orgCursor = undefined;
                let orgHasMore = true;
                while (orgHasMore) {
                    const orgResp = await this.withRetry('udesc.fetchOrganizations', () => this.udescClient.fetchOrganizations({
                        cursor: orgCursor,
                        pageSize: 100,
                    }));
            ...
            try {
                ...
                while (ticketHasMore) {
                    const ticketResp = await this.withRetry('udesc.fetchTickets', () => this.udescClient.fetchTickets({
                        cursor: ticketCursor,
                        pageSize: 100,
                    }));
        ...
        try {
            ...
            while (hasMore) {
                ...
                if (resp.records.length > 0) {
                    await this.prisma.$transaction(resp.records.map((record) => this.prisma.zouwuRequirement.upsert({
                        where: { id: record.id },
                        create: {
                            id: record.id,
                            title: record.title,
                            ...
                            status: client_1.RequirementStatus[record.status],
    ...
    async getZouwuFeedbackStatistics(query) {
    ...
    async updateSyncConfig(source, payload) {
        const intervalHours = payload.intervalHours !== undefined
    ...
    async smartFix() {
        ...
        const wrongVotes = await this.prisma.$queryRaw `
...
exports.SyncService = SyncService = SyncService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        udesc_client_1.UdescClient,
        zouwu_client_1.ZouwuClient])
], SyncService);
