 ...
 */
export async function fetchReportData(startDate?: string, endDate?: string): Promise<{
  ...
  const [kpiOverview, demandOverview, funnel] = await Promise.all([
    ...
    apiClient.get<ConsultationFunnelOverview>('/kpi/consultation-funnel', { params: { startDate, endDate } }).then(r => r.data).catch(() => null),
  ]);
