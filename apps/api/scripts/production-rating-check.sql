...
-- 1. 检查 UdeskSession 表中 4 月的评分情况
SELECT 
  'UdeskSession.rating IS NOT NULL' AS metric,
  COUNT(*) AS count
FROM "UdeskSession"
...
-- 2. 检查 UdeskSessionVote 表中 4 月的评分情况
SELECT 
  'UdeskSessionVote exists' AS metric,
  COUNT(*) AS count
FROM "UdeskSessionVote" v
JOIN "UdeskSession" s ON s."id" = v."sessionId"
WHERE s."startedAt" >= '2026-04-01' AND s."startedAt" < '2026-04-22';

-- 3. 检查 UdeskSessionVote.rating 不为空的数量
SELECT 
  'UdeskSessionVote.rating IS NOT NULL' AS metric,
  COUNT(*) AS count
FROM "UdeskSessionVote" v
JOIN "UdeskSession" s ON s."id" = v."sessionId"
...
SELECT 
  ...
  s."startedAt"
FROM "UdeskSession" s
LEFT JOIN "UdeskSessionVote" v ON v."sessionId" = s."id"
WHERE s."startedAt" >= '2026-04-01' AND s."startedAt" < '2026-04-22'
  AND v."rating" IS NOT NULL
